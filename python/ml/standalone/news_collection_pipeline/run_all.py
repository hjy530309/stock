...
SECTOR_CONFIG = {
    ...
    "엔터/미디어": {
        "search_query": "엔터|아이돌|K팝|에스엠|와이지|콘서트|빌보드|음원",
...
def verify_sector_fn(proc: ArticleProcessor, title: str, text: str, info: dict) -> bool:
    core_keys = info.get("search_query", [])
    special_sectors = ["2차전지", "방산/우주항공", "금융/밸류업", "엔터/미디어"]

    # 지정된 특별 섹터인 경우
    if any(sector in core_keys for sector in special_sectors):
        return True
...
async def main():
    ...
    iteration = 1
    wait_seconds = 600  # 10분 대기 (원하는 주기로 변경 가능)

    while True:
        print(f"\n[실행 {iteration}회차] 뉴스 수집 파이프라인 가동...")
        ...
        print("-" * 60)
        print(f"  [{iteration}회차 완료] news_data_sec: {sec_count}건 | news_data: {comp_count}건")
        print("-" * 60)

        iteration += 1

        # 다음 실행 전까지 대기 (API Rate Limit 및 서버 리소스 보호)
        print(f"다음 수집까지 {wait_seconds}초 대기합니다...\n")
...
if __name__ == "__main__":
    try:
        ...
        print("\n" + "=" * 60)
        print("  사용자(Ctrl+C)에 의해 수집 프로그램이 안전하게 종료되었습니다.")
        print("=" * 60)
